  ...
  nodes: [
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '业务方提了「工单智能分派」，本季度末要上线。请你按产品经理口径收口需求边界：哪些本期做、哪些砍掉，验收口径必须可衡量。' },
          { from: 'agent', text: '先确认一个点——分派准确率 95% 是线上口径还是测试集口径？这两个验收难度差很远，口径不定我没法排边界。' },
          { from: 'twin', text: '线上口径，取 7 天滚动窗口；测试集只作自测参考，不进验收。' },
          { from: 'agent', text: '明白。那我按线上口径收边界：规则引擎 + 分派执行链路做，人工改派和策略实验砍掉——这两个都会稀释线上准确率。' },
        ],
        tasks: [
          '核对业务方提出的 3 项诉求',
          '判定本期范围与非本期范围',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '产品那边把范围收在「规则引擎 + 分派执行」，验收是线上 7 天滚动准确率 95%。提醒你一句：范围一旦再扩，6 周排期直接失守——评估的时候把返工口子堵死。' },
          { from: 'agent', text: '收到。但有个前提要跟你对齐：要做到线上 95%，规则引擎不能只做静态规则，得留在线调优的入口。这会把后端工作量从 5 人日抬到 13 人日。' },
          { from: 'twin', text: '调优入口必须留，但配置界面不做。用配置文件下发，够用就行。' },
          { from: 'agent', text: '那就可行了。分派执行直接复用现有工单服务，避免双线改动。工作量增量 8 人日，我跟项目经理同步。' },
          { from: 'agent', text: '另外接口契约请前置到第 1 周冻结，否则前端会串行空等。' },
        ],
        tasks: [
          '拆解实现路径',
          '找出可复用的现有服务',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '架构师给的结论是后端 +8 人日，这是本期最大进度风险。请你据此重排期，把能压缩的砍掉。' },
          { from: 'agent', text: '8 人日没处挤。我只能砍需求，不能砍测试——砍测试等于把风险推到上线以后，我不接这个。' },
          { from: 'twin', text: '同意，测试不砍。砍规则配置界面，改只读展示。' },
          { from: 'agent', text: '这样能对齐 6 周。另外我要求接口契约第 1 周冻结并写进排期，否则前端串行空等，又是一天的坑。' },
        ],
        tasks: [
          '按 8 人日增量重排期',
          '砍掉可压缩的需求项',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '契约已按第 1 周冻结，你按契约实现规则引擎。上游提醒过：契约每延迟 1 天你就顺延 1 天，所以别等前端。' },
          { from: 'agent', text: '已经在按契约写了。但有个问题：条件求值的规则重叠边界，契约里没定义。我得自己定一套，定完要回写契约文档——不然前端会按老理解联调，白干。' },
          { from: 'twin', text: '可以，你定完直接更新契约文档，我来通知前端。' },
          { from: 'agent', text: '另外条件求值最容易出错，我先给它补单测再往下做，这会花掉半天。' },
        ],
        tasks: [
          '按冻结契约实现三个模块',
          '为条件求值补单元测试',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '后端落后 1 天，你的联调窗口被压缩了。契约里「条件求值」那部分刚更新过，注意看新版。' },
          { from: 'agent', text: '那我先用 mock 把看板跑通，不等联调。这样压缩的只是联调那一段，不是整个开发周期。' },
          { from: 'twin', text: '可以。但接口只留必需的，别铺开。' },
          { from: 'agent', text: '本期只联 4 个接口，够按工单状态筛选就行，其余不动。' },
        ],
        tasks: [
          '用 mock 先把看板主体跑通',
          '只联必需的 4 个接口',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '联调刚通，缓冲已经用完了。项目经理的要求是不削减用例、按优先级排序——请你按这个原则执行。' },
          { from: 'agent', text: '不削减我同意。但 32 条主流程用例全跑一遍要 2 天，会把上线窗口直接顶掉。我要求按阻塞级优先，边界与异常用例放第二轮。' },
          { from: 'twin', text: '可以。但主流程必须全跑完才能出结论，这个不能打折。' },
          { from: 'agent', text: '还有一件事要先说清楚：高并发场景这次我不做——那需要独立环境和压测窗口，不在当前排期里。如果需要，得单独排。' },
        ],
        tasks: [
          '主流程 32 条用例回归',
          '阻塞级缺陷同步后端修复',
      ...
      revisit: {
        exchange: [
          { from: 'twin', text: '审批驳回，退回原因我原文转达：「上线结论未覆盖规则分派的高并发场景，需补充压力测试后重新提交。」本轮只补压力测试，不重复功能回归。' },
          { from: 'agent', text: '收到。但压测要独立环境，之前没申请——现在申请要占掉半天。另外我要求这次一次做完，不要分两轮。' },
          { from: 'twin', text: '环境我今天内协调到位。压测一次做完。' },
          { from: 'agent', text: '明白。模拟 500 TPS 并发分派，重点验证规则冲突会不会在并发下复现——这是这次被驳回的核心疑点。' },
        ],
        tasks: [
          '申请独立压测环境',
          '500 TPS 并发压测',
  ...
  nodes: [
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '产线 8 月底试运行，需要提报工控机采购需求。请你把规格和到货节点写清楚，采购要按这个倒排。' },
          { from: 'agent', text: '有一个点要你拍：OPC UA 接口是硬条件还是可选？写硬条件会排除掉一批便宜型号；写可选，后面和 PLC 对接不上还是要返工。' },
          { from: 'twin', text: '写硬条件。选型阶段省下的钱，集成阶段要加倍还回来。' },
          { from: 'agent', text: '好。数量我按 3 条产线 × 6 台，加 2 台备件，共 20 台。到货节点 8/15，我标为不可让步。' },
        ],
        tasks: [
          '核对产线台数与备件数量',
          '把 OPC UA 锁定为硬条件',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '需求方把 8/15 定为不可让步，倒排后只剩 5 周——只有有现货的供应商能进候选。' },
          { from: 'agent', text: '问题在准入名单里：A 家报价最低，低 6%，但交期 30 天，赶不上。B、C 家有现货，但都贵。' },
          { from: 'twin', text: '那就先用交期过滤，再比价。不要反过来只看价格。' },
          { from: 'agent', text: '明白。过滤完有效报价只剩 2 家。A 家的排除原因我会写进结论，免得后面有人问为什么没选最低价。' },
        ],
        tasks: [
          '向准入名单 3 家发出询价',
          '按交期过滤失效报价',
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: 'A 家因为交期被排除了，现在只剩 B、C 两家。请你核技术符合性。有一句说在前头：不能为了赶交期放宽技术标准。' },
          { from: 'agent', text: '同意。但我要先提醒你：两家都合格，采购后面议价才有空间；如果只剩一家，谈价会非常难。这一点你审批的时候要考虑。' },
    ...
    },
    {
      ...
      run: {
        exchange: [
          { from: 'twin', text: '技术评估的结论是 B 家唯一合格，议价空间被压掉了。你的筹码只剩交期——用交期换价格，但财务和法务的基线不能让。' },
          { from: 'agent', text: '试过了。B 家愿意把交期从 30 天压到 18 天，但要求价格上浮 6%，而且违约条款给的赔付比例低于公司基线。' },
      ...
      revisit: {
        exchange: [
          { from: 'twin', text: '审批驳回，退回原因原文：「供应商不接受修订后的违约条款，需重新谈判并确认最终报价。」这次重谈，条款是前置条件，不拿来换价格。' },
          { from: 'agent', text: '那谈判顺序要改：先锁条款，再谈价。但供应商可能反过来要求加价，你要给我一个上限。' },
