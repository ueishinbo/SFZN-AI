...
import { PROCUREMENT_GROUP, PROCUREMENT_POSITIONS, PROCUREMENT_PROCESS, PROCUREMENT_ORG } from './procurementDemo';
...
export default function DefinitionWorkspace({ page, onConfigure, onTrial }: { onTrial: () => void; page: 'organizationDefinition' | 'processDefinition'; onConfigure: (position: Position) => void }) {
  ...
  const viewedProcess = purchaseView ? PROCUREMENT_PROCESS : LOCAL_PROCESS;
  ...
  const project = selected === 'procurement' || selected === 'demo' || !!sampleProject || selected.startsWith('project-');
  const orgName = selected === 'procurement' ? PROCUREMENT_GROUP : sampleDepartment ? sampleDepartment.name : sampleProject ? sampleProject.name : selected === 'demo' ? DEMO_ORGANIZATION : selected.startsWith('project-') ? organizations[Number(selected.slice(8))] : ORGS.find(o => o.id === selected)?.name || '公司组织';
  const positions = selected === 'procurement' ? PROCUREMENT_POSITIONS : sampleProject ? sampleProject.positions.map((name, i) => ({ id: `${sampleProject.id}/position-${i}`, org: sampleProject.id, name })) : selected === 'demo' ? DEMO_POSITIONS : selected.startsWith('project-') ? [] : COMPANY_POSITIONS.filter(p => p.org === selected || p.org.startsWith(selected + '/'));
  ...
  const peopleFor = (position: Position): { id: string; name: string }[] => {
    ...
    if (position.org === PROCUREMENT_ORG) return store.people.filter(p => p.id === `procurement-person-${position.id.split('-').at(-1)}`);
  ...
  return <div className="rc rc-shell definition-shell">
    ...
    {isOrganization ? <div className="definition-layout">
      <aside className="rc-section definition-tree"><header><h2>组织架构</h2><button className="rc-icon" aria-label="添加项目组织" onClick={() => { setName(''); setModal('organization'); }}><Plus size={18}/></button></header>
        ...
        <small>项目组织</small><button className={selected === 'procurement' ? 'selected' : ''} onClick={() => setSelected('procurement')}><UsersRound size={16}/>{PROCUREMENT_GROUP}</button><button className={selected === 'demo' ? 'selected' : ''} onClick={() => setSelected('demo')}><UsersRound size={16}/>{DEMO_ORGANIZATION}</button>
    ...
    </div> : <section className="rc-section"><header><div><h2>全部流程</h2><p>{2 + SAMPLE_FLOWS.length} 条组织流程 · 2 条已定义 · {SAMPLE_FLOWS.length} 条草稿</p></div><div className="rc-actions"><button onClick={() => setModal('import')}>导入</button><button className="rc-primary" onClick={() => setModal('create')}><Plus size={16}/>新建流程</button></div></header><div className="definition-table-scroll"><table className="definition-table"><thead><tr>{['流程名称', '编码', '所属组织', '关联岗位', '节点数', '状态', '操作'].map(t => <th key={t}>{t}</th>)}</tr></thead><tbody><tr><td><strong>{PROCUREMENT_PROCESS.name}</strong></td><td>FLOW-PUR-001</td><td>{PROCUREMENT_GROUP}</td><td>需求管理、策略、采购执行</td><td>3</td><td><Badge tone="green">已定义</Badge></td><td><div className="rc-actions"><button onClick={() => { setPurchaseView(true); setModal('view'); }}>查看</button><button onClick={onTrial}>试运行</button></div></td></tr><tr><td><strong>{LOCAL_PROCESS.name}</strong></td><td>FLOW-001</td><td>{DEMO_ORGANIZATION}</td><td>产品经理、项目经理</td><td>{LOCAL_PROCESS.nodes.length}</td><td><Badge tone="green">已定义</Badge></td><td><div className="rc-actions"><button onClick={() => { setPurchaseView(false); setModal('view'); }}>查看</button><button disabled title="本次演示暂不开放编辑">编辑</button></div></td></tr>{SAMPLE_FLOWS.map(flow => <tr key={flow.code}><td><strong>{flow.name}</strong></td><td>{flow.code}</td><td>{flow.organization}</td><td>{flow.positions}</td><td>—</td><td><Badge tone="orange">草稿</Badge></td><td><div className="rc-actions"><button disabled title="流程节点待配置">查看</button><button disabled title="本次演示暂不开放编辑">编辑</button></div></td></tr>)}</tbody></table></div></section>}
    {modal === 'organization' && <Dialog title="添加项目组织" onClose={() => setModal(null)} footer={<><button onClick={() => setModal(null)}>取消</button><button className="rc-primary" disabled={!name.trim() || [PROCUREMENT_GROUP, DEMO_ORGANIZATION, ...SAMPLE_PROJECTS.map(p => p.name), ...organizations].includes(name.trim())} onClick={addOrganization}>创建组织</button></>}><label className="definition-field">组织名称<input value={name} maxLength={40} onChange={e => setName(e.target.value)} placeholder="例如：XX 验证小组"/></label><p>项目组织独立管理，不改变成员的公司组织归属。</p></Dialog>}
    ...
    {modal === 'view' && <Dialog title={viewedProcess.name} wide onClose={() => setModal(null)}><div className="definition-flow-summary"><Badge tone="green">当前后台 · 手动定义</Badge><span>{purchaseView ? PROCUREMENT_GROUP : DEMO_ORGANIZATION}</span><span>{viewedProcess.nodes.length} 个节点</span></div><PositionCapabilityMap definition={{ ...baseDefinition('产品经理智能体', store.adminId), position: purchaseView ? PROCUREMENT_POSITIONS[0] : DEMO_POSITIONS[0] }} initialSource="local" processView/></Dialog>}
  </div>;
}
