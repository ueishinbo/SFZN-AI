...
可添加范围和 Demo 的“可见范围”使用同一套选择器，不另做人员表格、受限模式或默认岗位配置：
...
[搜索组织 / 小组 / 人员]
[多选结果列表]
已选择：上海飞机设计研究院、适航审定室、张伟
```

- 三个按钮是切换选择来源的分段控件，样式、搜索和多选交互复用 Demo；
- 可分别在组织、小组、人员三个来源中选择多个对象；切换来源不清空此前已选项；
- 保存后范围写入当前草稿；当前版本发布后，命中范围的人员才可在个人前台“可添加岗位智能体”列表中看到该对象，主动添加后才能使用；
- 系统不生成复杂的逐人绑定配置表，不维护默认岗位，不提供“受限模式”；
- 当人员组织或小组变化时，系统按当前组织与小组关系重新计算是否命中范围；
- 只有已发布岗位智能体的范围才会在个人前台生效。草稿、评测中、待审批状态可以编辑范围，但不会对人员开放。
...
| 状态 | 编辑 | 试运行 | 提交审批 | 编辑可添加范围 | 正式执行 |
| --- | --- | --- | --- | --- | --- |
| 草稿 | 可以 | 可以 | 不可以 | 不可以 | 不可以 |
| 评测中 | 只读；需撤回后修改 | 可以 | 评测通过后可以 | 不可以 | 不可以 |
| 待审批 | 不可以 | 只读查看证据 | 已提交 | 不可以 | 不可以 |
| 已发布 | 不可原地编辑 | 可以 | 不适用 | 可以 | 可以 |
| 已停用 | 不可以 | 不可以 | 不适用 | 不可以 | 不允许新任务 |
...
| 区域 | 内容与规则 |
| --- | --- |
| 左栏：适用组织 | 仅展示组织架构树，以组织节点作为唯一选择单位；提供展开/收起、组织人数、勾选状态和已选组织摘要。不展示人员头像、姓名、职级或个人卡片，也不提供小组、指定人员入口。它只负责决定谁能在个人前台添加该智能体，不是后台导航，也不负责编辑岗位内容。 |
...
人员绑定区块直接复用参考 Demo 的可见范围组件，标签改为“可添加范围”。不建设“人员绑定列表”“逐人暂停/解除”“批量绑定确认页”。
...
1. 默认进入“按组织”，可切换“按小组”和“指定用户”；
2. 每个来源支持搜索、多选、已选项回显与取消；
3. 选择组织、小组、人员后都写入同一个范围对象，切换 Tab 不清空已选项；
4. 草稿状态可编辑；评测中、待审批和已发布状态只读；已发布版本要修改范围，必须创建新版本；
5. 当前发布版本的范围是个人前台的可添加范围；人员命中任一项即可看到并主动添加该岗位智能体，添加后才可使用；
6. 范围中移除某个组织、小组或用户后，该人员在新版本发布前不受影响；新版本发布后按新范围生效；
7. 当前页面只展示“范围摘要”和“当前可添加人数”两个结果，不展示逐人绑定记录；
8. 个人对岗位智能体的添加、启停和移除属于前台个人配置，不在后台人员范围区块操作。
...
| 修改可添加范围 | 范围摘要和命中人数更新 | 新版本发布后，个人前台按新范围更新 |
