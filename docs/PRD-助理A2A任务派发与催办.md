...
5. 正式外部动作必须通过助理主区域底部的用户决策面板确认，不以内联消息卡代替；
6. Workspace 用于查看文件和任务投影，不承担第二套任务管理系统；
7. 紧急待办可通过对话内摘要、通知或首页提醒触达，但不以常驻任务面板为前提。

决策面板的出现时机、模型输出门控、多事项排队、关闭暂存和 Workspace 联动，以《PRD-A2A用户决策与确认机制》为准；如本文旧描述与其冲突，以该子规范为准。
...
2. 中间个人助理长 Session；
3. 右侧统一 Workspace；
4. 对话内任务查询列表和结果消息，以及聊天区可见时占用输入器位置向上展开、中窄屏 Workspace 独占时固定在主区域底部的用户决策 Dock。
...
- 优先级；
- 业务来源；
- 记录模式：写入 C 项目管理平台并生成协作投影，或仅创建助理本地协作任务；
- 附件；
- 预计写入和通知渠道；
- 权限或风险提示。
...
1. 服务端重新校验身份和权限；
2. 创建 `collaboration_task_id` 并确定 `record_mode`；
3. 如果是 `c_project_bound`，创建或更新 C 项目管理平台任务并保存 `external_task_id` 映射；
4. 如果是 `assistant_local`，由协作任务服务保存权威任务，不调用 C 项目管理平台；
5. 生成 A2A 事件；
6. 等待接收方服务送达确认；
7. 更新发起方对话结果；
8. 刷新双方任务文件投影。
...
- 确认页明确展示将采用 `c_project_bound` 还是 `assistant_local`；
...
- 在对话中展示简洁任务查询结果；需要用户处理的正式动作使用轻量用户决策 Dock；
- 不展示固定“今日关注”、Demo 剧本按钮或其他专用演示入口，所有剧本均通过自然语言触发；
- 派任务剧本支持选择唯一执行人、核对任务并确认派发；
- 选人默认展示三名推荐人员，点击“选择其他成员”后才展开组织搜索；
- 收到催办剧本模拟模型流式输出，A2A 决策等待输出结束后再弹出，面板不单独展示截止时间和最近进展摘要块；
- 聊天区可见时决策 Dock 占用输入器位置并向上展开；中窄屏 Workspace 独占时固定在主区域底部，不使用全屏遮罩或背景模糊；
- 增加连续澄清剧本，用 `1/3` 等步骤补齐评审范围、时间和准备方式；完成澄清只生成方案，不直接授权外部动作；
- “稍后处理”或 `Esc` 只表示暂存，并更新任务文件的决策状态；
...
- 用户决策 Dock 是否能在保持现场的同时形成足够明确的处理压力；
- 模型输出门控和关闭暂存语义是否符合用户预期。
