...
本文档所述的一期能力不是开放式 Agent 群聊，也不在 COMAC AI 内新建一套项目、成员和任务主数据。主持人制、严格顺序发言的 A2A 群聊属于另一条受控协作能力，详见《PRD-助理A2A群聊》；两者共享助理入口，但任务状态与群聊轮次彼此独立。
...
正式产品与当前前端 Demo 均不提供“切换接收方”按钮；每个用户只看到自己的助理。双边链路通过 A2A 异步回调与双方各自的消息记录体现，不在同一客户端跨身份演示。
